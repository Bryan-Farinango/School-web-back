...
class EstadoFirma extends Eloquent

{
    protected $collection = 'de_enum_estados_firmas';
    protected $fillable = ['_id', 'id_estado', 'estado'];

    public static function get_options_estados_firmas($id_estado = -1, $index_0 = true, $texto_index_0 = 'Todos los estados')
    {
        $selected = ($id_estado == -1) ? ' selected="selected" ' : '';
        $options = $index_0 ? ('<option value="-1" ' . $selected . '>' . $texto_index_0 . '</option>') : '';
        $estados = EstadoFirma::all(["id_estado", "estado"]);
        if (count($estados) > 0) {
